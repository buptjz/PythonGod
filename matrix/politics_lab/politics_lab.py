...
def create_voting_dict():
    ...
    for line in voting_data:
        items = line.split()
        dict[items[0]] = [int(no) for no in items[3:]]
    return dict
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    [v_a[i] * v_b[i] for i in range(len(v_a))]
    return sum([v_a[i] * v_b[i] for i in range(len(v_a))])
...
def most_similar(sen, voting_dict):
    ...
    """
    simDic = [(otsen,policy_compare(sen,otsen,voting_dict)) for otsen in voting_dict.keys() if sen != otsen]
    simName = ''
    simDegree = -3000
    for k,v in simDic:
        if v > simDegree:
