...
def downloadUrl(id,url,priQueue,downlist,beginurl):
    ...
    try:
		...
		pass
	    bodypattern=re.compile(ur'''\xe5\x8f\x91\xe4\xbf\xa1\xe4\xba\xba:(.+?), \xe4\xbf\xa1\xe5\x8c\xba:(.+?)<br /> \xe6\xa0\x87&nbsp;&nbsp;\xe9\xa2\x98:(.+?)<br />.+?\((.+?)\),.+?<br />&nbsp;&nbsp;<br />(.+?)\[FROM:''',re.DOTALL)
