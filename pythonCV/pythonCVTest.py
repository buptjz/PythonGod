...
def tag(keypoints,img):
    ...
    for key in keypoints:
        ...
        sx = int(key.pt[0])
        sy = int(key.pt[1])
        ex = int(sx + r * cos(angle/180. * pi))
        ey = int(sy + r * sin(angle/180. * pi))
        cv2.line(img, (sx,sy), (ex, ey), cv.RGB(0, 255, 0), 1,cv2.CV_AA)
    return img
...
def match():
    ...
    tagImage = tag(skp,img)
    cv2.imshow("image", tagImage)
    cv2.waitKey(0)
